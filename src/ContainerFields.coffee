addFields = (fields) ->
  ...
  class BaseContainerField extends fields.Field
    ...
    _procOpts: (opts) ->
      opts ?= {}
      if _.isString(opts) or _.isArray(opts) then opts = {path: opts}
      return opts
  ...
  class ContainerField extends BaseContainerField
    ...
    setValue: (val, opts) ->
      ...
      origValue = @getValue()
      if val == undefined then val = _.clone(@default) || {}
      if not val or _.isEqual(val, origValue) or not @_fields then return
      if not _.isObject(val) or _.isArray(val) then throw JSON.stringify(val) + " must be a hash"
      @value = val
      _fields = @getFields()
  ...
  class HashField extends ContainerField
    ...
    setValue: (val, opts) ->
      ...
      if opts?.path?.length then return @_applyToSubfield("setValue", opts, val)
      if val == undefined then val = _.clone(@default) || {}
      if not val or not @schema or _.isEqual(val, @getValue()) then return
      if not _.isObject(val) or _.isArray(val) then throw JSON.stringify(val) + " must be a hash"
      @resetFields()
      @value = val
  ...
  class ListField extends BaseContainerField
    ...
    setValue: (val, opts) ->
      ...
      if val == undefined then val = _.clone(@default) || []
      if not val or not @schema or _.isEqual(val, @getValue()) then return
      if not _.isArray(val) then throw JSON.stringify(val) + " must be an array"
      @resetFields()
      @value = val
      for value in val
        @_addField(@schema, value)
